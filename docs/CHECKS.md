...
`node scripts/check-all.mjs` は `scripts/check-*.mjs` の計34ファイルと `docs/check.mjs` の1ファイル、合計35本を `spawnSync` で直列に呼ぶだけの外側の殻です。各 check 本の中身はこのタスクでは無編集(AUTOPILOT の運用どおり)。

## 結果の3値と「再実行は最大1回」の方針(R223)

`check-all.mjs` の結果欄は **PASS / ★FLAKY / FAIL の3値**です。

| 結果 | 意味 | 終了コードへの影響 |
|---|---|---|
| `PASS` | 1回目で通った | 緑(exit 0) |
| `★FLAKY(2回目で通った)` | **1回目は落ちたが、5秒待って1回だけやり直したら通った** | 緑(exit 0)。ただし表・専用の一覧・最終行の3か所に必ず出る |
| `FAIL` | **2回走らせて2回とも落ちた** | 赤(exit 1) |

- **再実行は1本につき最大1回(合計2回)まで**。2回目も落ちたら必ず赤にします。何度でもやり直す作りにしないのは、**自動再実行が本物の不具合を永久に隠す道具になりうる**ためです。回数を増やす変更はしないこと。
- **FLAKY は「なかったこと」にしません**。最終行は `35本中 35本 PASS(うちFLAKY 1本)` のように**本数を数字で出す**ため、フレークが増えていることに気付けます。FLAKY が続く本は、再実行で隠している間に本物の不具合が育っている可能性があるので、1回目のログを読むこと。
- **1回目の失敗ログは2回目が通っても必ず残します**。`screenshots/fail-<本名>-<日時>-try1.txt`(2回目も落ちたときは `-try2.txt` も)。再実行の判断に使った証拠を捨てないためです。
- **待ち時間は5秒**(`RETRY_WAIT_MS`)。フレークの主因がWindowsの一時ポート枯渇(`ERR_NO_BUFFER_SPACE`)で、ソケットが `TIME_WAIT` から片付くまでの時間を与える意図です。待たずに即やり直すと同じ理由で落ちるだけになります。
- **この仕組みは呼び出し側(`check-all.mjs`)だけで完結**しており、35本の検査スクリプト自体は1バイトも変えていません。実行順序も判定内容も変わりません。

## サーバを立てる30本(全て Playwright あり)
...
| check-bundle | カードを「テーマの束」にまとめた見出し(.feedbundle)の検査(R157/R158)。展開前は0本・展開後は1本以上・中身1件の束が無いこと・見出しの下のカードが全て同じテーマであること・番号バッジ/data-indexの過不足なし・初期5件がrank順のまま先頭に並ぶこと等を検査。**(R222)** 照合表を組み立てるために `assets/geo.js` `assets/engine.js` から定義(`CATEGORY_LABELS` `WIKI_CATEGORY_HINTS` `FALLBACK_THEME` `THEME_OF`)を正規表現で読むが、**JSのコメント(`//` と `/* */`)を除いてから判定する**。除かないと定義をコメントアウトしても「ある」と誤判定して緑のまま通るため(R214 と同種の穴)。除去は1文字ずつ読む方式で、文字列リテラル・テンプレートリテラル・正規表現リテラルの中の `//`(例: `'https://…'`)は消さない |
...
| check-demo | R218: 営業デモ `demo/hotel-page-en.html` と `demo/embed-check.html` をブラウザで開き、HTTP200・`<title>`・`lang`・JSエラー0件・iframe が `embed=1`+`fixture=kusatsu` で実描画されカードが出ること・en側の主要見出し/日本語版への導線/OGタグ4種/高さ通知(640px超)・embed-check の説明文3本以上を検査(この2枚は既存の他の検査がブラウザで開いていなかった) |
| check-distance | 距離表示・並び順 |
| check-fame | R229: カードの知名度バッジ(`.feedcard__fame`)。R224 で入れた表示を守る本で、5エリアの固定データについて **(1)** バッジ付きが1枚以上かつ全枚数未満(全部に出る/1枚も出ないのどちらでも落ちる) **(2)** `_debug.backlinks` が 0・欠落・60未満のカードに出ていないこと(**「人気0」という存在しない評価を出さない**のが R224 で一番避けたかった事故) **(3)** 理由行が「このあたりの代表的な」で始まるカードに地方級バッジが同居していないこと(同じ範囲の話を2回しない。除外を外すと5エリアで10枚復活する) **(4)** 文言が `★ 全国的によく知られた場所` / `★ この地方でよく知られた場所` の2種類のみ・1カード最大1個 を検査する。しきい値 150/60 は検査側にも定数で持ち、`assets/app.js` から**コメントを除去してから**(R222 と同じ方式)正規表現で読んだ値と一致するかも見るため、片方だけ動かすと落ちる。外部APIは叩かない |
...
| check-noscript | R221: JSが無効なときの案内(R215 で入れた `index.html` の `<noscript>`)。`javaScriptEnabled: false` で開いて本文が200文字以上あること・「JavaScript」「再読み込み」「やどたび」の3語が入ること・本番URLへのリンクがあることを検査し、JS有効時には noscript の中身が見えずカードが描画されることも確認する(33本すべてがJS有効前提で、JSを切って開く検査が1本も無かった) |
...
所要目安(現在値・2026-09-20 R229 実測): **35本**(check-all.mjs)の合計は **382.8s(Windows)**、最遅は **`check-reason.mjs 29.9s`**、**FLAKY 0本**。R229 で `check-fame`(**15.5s**)を1本追加して35本になった。直前の R223 実測は追加前の34本で **350.1s**、最遅は **`check-reason.mjs 28.2s`**。この回は `check-chipcurrent` が `ERR_NO_BUFFER_SPACE` で1回目に落ち、再実行で通った(FLAKY 1本)ため、再実行分(5秒の待ち+8秒の実行)が含まれている。**再実行が1度も起きなければ R221 時点の348.1s からほぼ増えない**。R221 実測は再実行機構を入れる前の **348.1s**、最遅は **`check-reason.mjs 28.4s`**。R221 で `check-noscript`(3.3s)を1本追加して34本になった(直前のR218実測は追加前の33本で348.3s)。R218 で `check-demo`(5.8s)を追加して33本になり、さらに直前のR205実測は追加前の状態で343.4s(Windows)/ 342.5s(CI)、CI(GitHub Actions)でもWindowsと同じ結果が出ることを確認済み。過去(check本数が少なかった頃)の記録はNIGHTLOGの当該サイクルを参照(この節は現在値だけを保持する運用にする)。
