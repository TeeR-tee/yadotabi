...
- 次: **修正後の実APIでの効果は未確認**(本物API呼び出しは1サイクル1回の規約を守り、退行検知に使い切ったため)。次サイクルの冒頭で箱根に対し1回だけ実行し、件数と最遠距離(修正前は32件・2971m、R17時点の旧実装は50件・3.7km)、および大涌谷(7.6km)・彫刻の森(5.2km)が入るかを記録してほしい。そのうえで R19(far 分布の是正)へ。
