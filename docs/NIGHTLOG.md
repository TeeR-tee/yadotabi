...
- **[L2327]** R199: 交通拠点(バスターミナル等)、実測したら既に表示圏外だった
  ...
  - [L2444] 2026-09-20 R227 理由行の優先順を「代表的な○○ → 珍しい○○ → 歩いて行ける」に入替えて同じ文言の重複を減らした
...
### 2026-09-20 R227 理由行の優先順を「代表的な○○ → 珍しい○○ → 歩いて行ける」に入替えて同じ文言の重複を減らした
- やったこと: `assets/app.js` の `reasonText()` の**呼び出し順だけ**を入替えた(各 Reason 関数の中身・しきい値 `REASON_BACKLINKS_MIN` は無変更)。温泉街は宿の徒歩圏に見どころが集まるため、旧順では上位5枚の **25枚中14枚(56%)** が「歩いて行ける」という同じ文言だった。草津は **5/5 → 2/5** に減り、上位3枚が全部違う文言になった。順位(rank)は1バイトも変えていない。
- **★司令塔の期待値(4枚)は不完全だった。実測では上位5枚のうち6枚が変わった**: 指示書の4枚(草津2位 草津熱帯圏/箱根2位 早雲寺/道後5位 子規記念博物館/別府3位 別府市美術館)は全て予想どおり `代表的な○○` になったが、加えて **草津1位 湯畑** と **草津3位 光泉寺** の2枚が `rareReason`(このあたりでは珍しい○○)に変わった。原因は、計画役が `representativeReason` と `walkableReason` の2つだけを比べており、**`rareReason` も同時に `walkableReason` の前に出ることを数え忘れていた**ため。なお「湯畑は変わらない」という指示書の記述も誤りで、`backlinks` が `null` なので「代表的な」にはならないが「珍しい」には当たる。**司令塔が渡した期待値も実測で確かめること**(R230 の教訓)がここでも生きた。
...
- 守る検査: 新規ファイルを作らず既存 `scripts/check-reason.mjs` に(e)(e2)を追記。**枚数は数えず**、材料(distanceM/backlinks/同カテゴリ枚数)から期待される文言を検査側で組み立て直し、1枚ずつ画面と突き合わせる。`REASON_BACKLINKS_MIN` は `app.js` から(コメント除去後に)読んで検査側の期待値と一致も見る。`check-reason.mjs` 単体は **37 pass / 0 fail**。
- **★わざと壊して赤くなることを4通り実証**: (a) 優先順を元に戻す→**8項目FAIL**、(b) `representativeReason` を常に `null` →**6項目FAIL**、(c) **文言だけを「この付近を代表する」に変える**(枚数は変わらないので数える検査では素通りする壊し方)→**5項目FAIL**、(d) `REASON_BACKLINKS_MIN` を 30→20 →**3項目FAIL**。4通りとも手で書き戻し、`md5sum assets/app.js` が実証前後で**同一**(`ab43177d…`)であることを確認(`git checkout`・`stash` は不使用)。
- R224 の人気バッジとの整合: 優先順を変えると「代表的な」のカードが増え、地方級バッジを抑える規則(R224/規則4)に該当する枚数が増える。地方級は **3枚 → 2枚** に減ったが「1枚以上実在」を満たし、`check-fame.mjs` は **43 pass / 0 fail** で緑。**検査の変更は不要だった**。
- 撮影: 固定データ5エリア(375px)を撮影し全枚を目視。別府3位「このあたりの代表的な美術館・博物館」(5エリアで最長)も**1行に収まりはみ出し無し**。理由行と★バッジが同じカードで冗長になっていないことも確認(別府3位は「代表的な美術館」+全国級バッジで語る範囲が違うため良し)。箱根1位が理由行無しなのは before も同じで R227 の影響ではない。
- 検証: `node scripts/check-all.mjs` **35本中35本PASS(★FLAKY 0本)・合計 403.8s・exit 0**(最遅 `check-reason.mjs` 41.8s)。**FLAKY は0本**(R223 の申し送りに従い記録。R229 0本・R230 0本・R228 0本・今回0本と、増えていない)。本数は増やしていない(既存検査への追記のみ)。`git diff --stat` の対象は `assets/app.js`・`scripts/check-reason.mjs`・`docs/ROADMAP.md`・`docs/NIGHTLOG.md` の**4ファイルのみ**で、`assets/geo.js` `assets/engine.js` `fixtures/` `demo/` `index.html` `assets/style.css` は**1ファイルも含まれない**。外部API消費**0回**。
