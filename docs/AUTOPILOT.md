...
8. 1サイクル = 1タスク。欲張らない。
9. 大きな変更のあとは、**本番(https://teer-tee.github.io/yadotabi/)で実際に宿を選んで確認する**。固定データでは速度・APIの失敗・住所の解析・宿名に依存する処理が検証できない(詳細は `docs/FIXTURES.md`)。
...
- 1サイクル3行: 「やったこと / 見た目の確認結果 / 次」。**必ずファイル末尾の「## サイクル記録」節の末尾に追記する**(ファイル先頭に新しい節を作らない。「## 朝のまとめ」が常に最初の節)。**箇条書きの直前に必ず `### 2026-MM-DD R<番号> <一言>` の見出しを付ける**(2026-09-16 R126 で、見出しの無い裸の箇条書きが36サイクル分積み上がり目次が効かなくなる事故があったため)。
