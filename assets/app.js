...
(function (global) {
  ...
  // 25枚中14枚(56%)が「歩いて行ける」という同じ文言になり、どれが何なのかが区別できなかった
  // (草津5/5・城崎4/5・箱根2/5・道後2/5・別府1/5 の実測)。距離は「徒歩◯分」がメタ行に既に
  // 出ているので、実績の裏づけがある「代表的な◯◯」は距離より先に言う価値がある。
  //
  // ★ただし rareReason(珍しい◯◯)は walkableReason より後ろに置く。
  // 「代表的な」は backlinks(被リンク数)という実績に裏打ちされた情報だが、
  // 「珍しい」は**同カテゴリが表示中に1件しかない**というだけの消去法的な事実で、
  // その場所の性格を何も語っていない。前に出すと代表格に付いて嘘になる:
  // 実際、一度 rareReason を前に出したところ草津1位の湯畑が
  // 「このあたりでは珍しい観光名所」になった(湯畑は backlinks が null なので
  // representativeReason の下限判定に到達せず、rareReason に落ちる)。
  // 湯畑は草津の代表格で「珍しい」は明確に誤りなので、徒歩圏という事実を先に出す。
  //
  // この順で文言が変わるのは上位5枚では4枚(5エリア25枚中)。いずれも代表的な◯◯になる:
  // 草津2位 草津熱帯圏(動物園)/箱根2位 早雲寺(神社・寺院)/
  // 道後5位 子規記念博物館(美術館・博物館)/別府3位 別府市美術館(美術館・博物館)。
  // 草津の「歩いて行ける」は 5/5 → 4/5 に減る。
  ...
  function fameText(card, reasonMsg) {
    var d = card && card._debug;
    if (!d) return null;
    // R241: 抑制条件が見る母集団は representativeReason() と同じ state.cards(初期5枚)。
    // ここを cards+more に広げると「代表的」を決めた母集団と判定がずれる。
    var cardsInView = Array.isArray(state.cards) ? state.cards : [];
    ...
    // R224: 「このあたりの代表的な◯◯」は“この土地の中での位置づけ”を言っている。
    // そこへ「この地方でよく知られた場所」を並べると同じ範囲の話を2回することになるため
    // 地方級を一律で出さないようにした。
    //
    // ★R241: この一律の抑制が、みのるんが最初に見る初期5枚に集中していた。
    // 5エリアの初期25枚のうち11枚が「このあたりの代表的な…」の理由行で、そのせいで
    // バッジが出ているのは 8/25枚(草津 0/5・箱根 3/5・道後 2/5・別府 2/5・城崎 1/5)。
    // 看板エリアの草津が 0/5 で、2位の草津熱帯圏は backlinks 129(地方級の基準60以上)
    // なのにこの1行だけで消えていた。
    //
    // 抑制を「同じ範囲の話が本当に重なるときだけ」に狭める。判定材料は
    // representativeReason() が「代表的」を決めるのに使った母集団そのもの(= cardsInView)。
    //   - 同じカテゴリのカードが母集団に2枚以上ある → 理由行は実際に他と比べて上だと
    //     言っている。そこへ知名度を足すと同じ「順位の話」を2回することになるので出さない。
    //   - 同じカテゴリが自分1枚だけ → 理由行は比べる相手がおらず「この画面で唯一の◯◯」と
    //     言っているだけで、どれだけ知られているかは1文字も言っていない。ここに
    //     「この地方でよく知られた場所」を出しても重複にならないので出す。
    // 文言は R224 の2種類のまま1つも増やしていない(出す条件だけを変えた)。
    if (reasonMsg && reasonMsg.indexOf('このあたりの代表的な') === 0 &&
        sameCategoryIn(cardsInView, card, card.categoryLabel).length >= 2) return null;
   ...
   *
   * ★R246: 直す前は「宿からの距離と種類の多様性で並べた暫定版です。有名な場所が
   * 下に来ることがあります。」と書いていたが、engine.js の WEIGHT を開くと順位に
   * 効いている要素は8つあり、そのうち最大の PARENT_MENTION(親記事でどれだけ
   * 語られているか)と BACKLINK_MAX(他の記事からどれだけ触れられているか)は
   * まさに「どれだけ知られているか」を順位に効かせている。つまり旧文は
   * 「有名さは順位に使っていない」と読める嘘になっていた。しかも同じ画面の上には
   * FAME_ORIGIN_TEXT(★は、百科事典で他の記事からどれだけ触れられているかで
   * 付けています)が出ており、1つの画面で逆のことを言っていた。
   *
   * 新しい文は実際に効いている4種類だけを名乗る:
   *   - 宿からの距離(DISTANCE_PER_KM)
   *   - 種類の多様性(CATEGORY_PENALTY)
   *   - どれだけ語られ触れられているか(PARENT_MENTION / BACKLINK_MAX)
   *   - 写真や説明が揃っているか(WIKI_IMAGE / WIKI_SUMMARY / SOURCE_BOTH / OFFICIAL_SITE)
   * 言い方は FAME_ORIGIN_TEXT の「触れられているか」に揃える(同じ概念を2通りの
   * 言葉で呼ばない)。数字・しきい値・評価の語・専門用語は1文字も入れない。
   */
  function noteHtml(cardCount) {
    if (!cardCount) return '';
    return '<p class="feednote">この提案は、周辺の地図情報（OpenStreetMap）とWikipediaから、' +
      '宿からの距離、種類の多様性、百科事典でどれだけ語られ触れられているか、' +
      '写真や説明が揃っているかを合わせて並べた暫定版です。' +
