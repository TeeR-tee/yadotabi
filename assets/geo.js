...
(function (global) {
   ...
   * R186: ただし宿名の次に **道路名** が入ることがある(草津の宿は
   * 「しゃくなげ通り」)。道路名は地名ではないので飛ばして次の要素を見る。
   *
  ...
  function parentArticleTitle(hotel) {
    ...
    for (var i = 1; i < parts.length; i++) {
      ...
      // R186: **道路名は地名ではない**ので飛ばす。Nominatim は宿名の次に street を
      // 入れることがあり(実測: 草津の宿は「しゃくなげ通り」「西の河原通り」)、
      // ここで止まると「しゃくなげ通り温泉」という実在しない記事を引いてしまい、
      // 親記事の加点が**エリアごと全滅**する(R185が見た草津の症状そのもの)。
      // 1つ飛ばせば正しい地名(草津)に届く。通り・道・街道・筋・線は地名の語尾には
      // 使われないため、温泉地名を巻き込む心配はない。
      if (/(通り|通|丁目|道|街道|筋|線|号線)$/.test(p)) continue;
