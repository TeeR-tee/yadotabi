...
| `?fixture=` | `kusatsu` / `hakone` / `dogo` / `random` | 保存済みの生レスポンスで画面を再現する(`random` は3エリアから実行時に1つ選ぶ) | 叩かない |
...
- `fixtures/` — `?fixture=` で使う保存済みの外部API応答データ(草津・箱根・道後の3エリア)。固定データの作り方・再生成の判断基準は [docs/FIXTURES.md](docs/FIXTURES.md) を参照。
