...
- **検索候補とエリアチップの重なり**: 候補のドロップダウンは検索欄の真下に出るため、その下にあるエリアチップ行に必ず重なります。いまは候補の背後からチップが半分はみ出して見えており、見た目が整理されていません。選択肢は (a) 候補が開いている間はチップを隠す (b) チップを薄くする (c) このままにする、の3つで未決です。
...
- **実APIとfixtureのWikipedia件数の食い違い**: 同じ中心(箱根湯本 35.2324,139.1069)・同じ半径10kmで比較すると、`fixtures/hakone.json` は50件(最遠3,720m)を保存していますが、直近の実API実測では34件(最遠3,574m)しか返ってきませんでした。つまり「50件の上限で結果が打ち切られている」という当初の前提が、現状では再現しません。選択肢は (a) fixtureを再生成して実APIに合わせる (b) 真因調査に1サイクルを使う (c) 深追いしない、の3つで未決です。
...
| `?bg=` | `?bg=fff7e6` | `?embed=1` と併用したときだけ有効。背景色を6桁の16進で指定し宿ページに馴染ませる(不正な値は無視) | 併用先に従う |
