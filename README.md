...
- **提案エンジン(`engine.js`)**: 「**集める(collect)→並べる(rank)→整形する(present)**」の3段構成になっています。collectでOSMとWikipediaの候補を集めて重複を1つにまとめ、rankでスコア順に並べ替え、presentで写真・1行要約・徒歩/車の分数・外部リンク付きのカードに整形します。
  - **正直な注意点**: `rank()` は現状まだ暫定実装です。順位を決めているのは、画面末尾の注記と同じ次の4種類です —— **宿からの距離**(`DISTANCE_PER_KM` 6/km の減点)、**種類の多様性**(`CATEGORY_PENALTY` 18 の減点)、**百科事典でどれだけ語られ触れられているか**(親記事がその場所を何回挙げたかで配る `PARENT_MENTION` 上限80 と、他記事からの被リンク数で配る `BACKLINK_MAX` 上限18)、**写真や説明が揃っているか**(`WIKI_IMAGE` 25 / `SOURCE_BOTH` 20 / `WIKI_SUMMARY` 15 / `OFFICIAL_SITE` 12)。このうち**一番重いのは親記事での言及(80点)**で、被リンクと合わせて「どれだけ知られているか」が順位に直接効きます。そのため結果はどうしても有名な観光地に偏ります(写真や公式サイトの有無も効きますが、合計52点で親記事の言及ひとつに届きません)。「まだ知られていない穴場を見つけて出す」ためのアルゴリズムは今後の研究課題であり、`rank()` はそのために差し替えやすいよう1関数に閉じ込めてあります。
...
- **カテゴリ多様性の減点をどこまで効かせるか**(2026-09-24 R247 で前提が変わりました): かつては減点 `18×(n-2)` が上限なく積み上がり、大涌谷−72・彫刻の森美術館−108で上位30枚に入らない、という論点でした。**現在は選択肢 (a)「減点に上限を設ける」が採用済みです**(`engine.js` の `PENALTY_CAP = 3`。同カテゴリ3件目で頭打ちになり、減点は最大 `18×(3-2+1)=36` 止まり)。**`dump-rank.mjs` で再実測したところ、箱根では彫刻の森美術館が初期5件の#4、大涌谷も上位30枚の中に出ており、「締め出される」という記述自体が現状と一致しません**(R247 で実測)。残っている論点は「頭打ちの位置(3件目)が適切か」だけで、この論点を続けるなら実測から取り直す必要があります。
...
### 写真があるカードの割合(fixture 上位30件・2026-09-24 R247 で再実測)

`node scripts/dump-rank.mjs <area>` の cards + more(= 上位30枠)を数えた実測値です。上位30枠は除外ルール通過後の件数なので、エリアによっては30件に満たない点に注意してください(2026-09-18 時点の旧表は母数を一律30件として書いていましたが、再実測では再現しませんでした)。

| エリア | 上位30枠の件数 | 写真あり | 割合 |
|---|---|---|---|
| 草津 | 24件 | 11件 | 46% |
| 箱根 | 22件 | 21件 | 95% |
| 道後 | 18件 | 16件 | 89% |
