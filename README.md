...
`node scripts/check-all.mjs` を実行すると、`scripts/check-*.mjs` 10本と `docs/check.mjs` を1コマンドで直列実行し、結果を表(PASS/FAIL・所要時間)で確認できます。1本でも失敗すると exit code 1 で終了します。一部の検査は内部でPythonの `python -m http.server` を一時起動するため、Python 3 が必要です。
